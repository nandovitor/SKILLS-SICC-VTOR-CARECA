...
def read_doc_with_antiword(file_path):
    ...
    if not antiword_path:
        raise RuntimeError("antiword não encontrado no PATH")
...
def extract(file_path):
    ...
    raise RuntimeError(f"Formato não suportado pelo helper Python: {suffix or 'sem extensão'}")


def main():
    if len(sys.argv) < 2:
        emit({"ok": False, "error": "Arquivo não informado."})
        return 1
